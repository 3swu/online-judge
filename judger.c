...
void prog_compile(){
    ...
    if(file_type == 0){
        ...
        sprintf(cmd, "gcc -O2 -w -fmax-errors=3 -std=c11 %s -lm -o %s", infile_path, outfile_path);
        system(cmd);
        ...
        char outfile_path[100];
        sprintf(outfile_path, "%s/%s", temdir_path, outfile_name);
        sprintf(cmd, "g++ -O2 -w -fmax-errors=3 -std=c++14 %s -lm -o %s", infile_path, outfile_path);
        system(cmd);
...
void execute_prog(){
    ...
    /*父进程*/
        ...
        while(1){
            ...
            else if(WIFSIGNALED(status)){ //信号退出
                if(WTERMSIG(status) == SIGSEGV){
                    if(mem_usage < memory_limit)
                        return_result("",RS_MLE);
                }
            }
...
int syscall_illegal(int callid){
    int white_list_length = sizeof(LANG_CV) / sizeof(int);
